...
impl PersonalWeChatChannel {
    ...
    async fn process_message(
        ...
        *self.last_contact.write().await = Some((from_id.clone(), context_token.clone()));
        ...
        // 🛡️ DEDUP FIX: Check seq-based local deduplication to prevent iLink duplicate
        // pushes
        ...
        if seq > 0 {
            ...
            if recent_seqs.iter().any(|(s, _)| *s == seq) {
                warn!(
                    "🔄 Duplicate iLink message detected (seq={}), skipping",
                    seq
                );
                return Ok(());
            }
...
impl Channel for PersonalWeChatChannel {
    ...
    async fn start_listener(&self, event_bus: mpsc::Sender<ChannelEvent>) -> Result<()> {
        // 🛡️ FIX: Serialize start_listener to prevent race conditions from concurrent
        // calls
