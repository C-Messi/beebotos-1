...
impl DagScheduler {
    ...
    async fn fail_task(
        ...
        let (should_replan, continue_on_failure) = if let Some(instance) =
            instances_guard.get_mut(&request.instance_id)
        {
            if let Some(state) = instance.task_states.get_mut(&request.task_id) {
                ...
                if state.retry_count < instance.workflow.config.retry_policy.max_retries {
                    ...
                    let should_replan = instance.workflow.config.enable_replanning
    ...
    /// Update downstream tasks after a task completes or fails with
    /// continue_on_failure
    async fn update_downstream_tasks(
                ...
                if let Some(instance) = instances_guard.get(instance_id) {
                    if let Some(state) = instance.task_states.get(task_id) {
                        if state.status == TaskExecutionStatus::Pending {
                            // Check if all dependencies are completed (or failed with
                            // continue_on_failure)
                            let deps = instance
                                .workflow
                                .dependencies
        ...
        let ready_task_clones: Vec<DagTask> = ready_tasks
            .iter()
            .filter_map(|task_id| {
                instances_guard
                    .get(instance_id)
        ...
        for (task_id, task) in ready_tasks.into_iter().zip(ready_task_clones.into_iter()) {
            ...
            queue.push_back(request);
            debug!(
                "Queued downstream task {} for instance {}",
                task_id, instance_id
            );
        }
    }
    ...
    async fn test_multi_level_dag_execution() {
        ...
        let completed = timeout(Duration::from_secs(5), async {
            while let Some(event) = receiver.recv().await {
                if let SchedulerEvent::WorkflowCompleted {
                    instance_id: id,
                    success,
                    ..
                } = &event
        ...
        assert!(
            completed.is_ok(),
            "Workflow timed out — downstream tasks were never queued"
        );
