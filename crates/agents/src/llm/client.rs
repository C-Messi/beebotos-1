...
impl LLMClient {
    ...
    /// and automatically handles tool calls until the model returns a final
    /// answer or max_rounds is reached.
    pub async fn chat_with_tools_react(
        ...
        for _round in 0..max_rounds {
            ...
            request.config.tools = Some(tool_handlers.iter().map(|t| t.definition()).collect());

            let response = self.provider.complete(request).await?;

            if let Some(choice) = response.choices.first() {
                if let Some(tool_calls) = &choice.message.tool_calls {
                    // 🆕 FIX: Constraint 2 - Preserve assistant message with reasoning_content in
                    // context for multi-step tool calls (required for Kimi k2.6
                    // thinking mode)
                    messages.push(choice.message.clone());

                    // Execute each tool call
                    for tc in tool_calls {
                        let result = if let Some(handler) = tool_handlers
                            .iter()
                            .find(|h| h.definition().function.name == tc.function.name)
                        {
