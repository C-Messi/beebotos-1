...
impl Default for ReActConfig {
    fn default() -> Self {
        Self {
            max_steps: crate::react_trace::DEFAULT_REACT_MAX_TOOL_ROUNDS as usize,
            ...
            if let Some(tool_call) = self.parse_tool_call(&response) {
                ...
                let observation = match self.tools.get(&tool_call.name) {
                    Some(tool) => match tool.execute(&tool_call.arguments).await {
                        Ok(result) => result,
                        Err(e) => crate::react_trace::format_tool_error(&tool_call.name, e),
                    },
                    None => {
                        let available: Vec<_> = self.tools.keys().cloned().collect();
                        crate::react_trace::format_unknown_tool_error(&tool_call.name, &available)
                    }
                };
