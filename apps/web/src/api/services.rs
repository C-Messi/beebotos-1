...
impl WorkflowService {
    ...
    pub async fn execute(
        ...
        self.client
            .post(&ApiEndpoints::WORKFLOW_EXECUTE.replace("{id}", id), req)
            .await
    }

    pub async fn dashboard_stats(&self) -> Result<DashboardStats, ApiError> {
        self.client
            .get(ApiEndpoints::WORKFLOW_DASHBOARD_STATS)
            .await
    }
    ...
    pub async fn recent_instances(
        &self,
        limit: usize,
    ) -> Result<Vec<WorkflowInstanceSummary>, ApiError> {
        self.client
            .get(&format!(
                "{}?limit={}",
                ApiEndpoints::WORKFLOW_DASHBOARD_RECENT,
                limit
            ))
            .await
    }
    ...
    pub async fn workflow_stats(&self, id: &str) -> Result<serde_json::Value, ApiError> {
        self.client
            .get(&format!(
                "{}/stats",
                ApiEndpoints::WORKFLOW_STATS.replace("{id}", id)
            ))
            .await
    }
    ...
    pub async fn install(
        &self,
        req: &InstallWorkflowRequest,
    ) -> Result<InstallWorkflowResponse, ApiError> {
    ...
    pub async fn uninstall(&self, id: &str) -> Result<serde_json::Value, ApiError> {
        self.client
            .post(
                &ApiEndpoints::WORKFLOW_UNINSTALL.replace("{id}", id),
                &serde_json::json!({}),
            )
            .await
    }
...
impl CompositionService {
    ...
    pub async fn get(&self, id: &str) -> Result<CompositionInfo, ApiError> {
        self.client
            .get(&format!(
                "{}{}",
                ApiEndpoints::COMPOSITION_DETAIL,
                js_sys::encode_uri_component(id)
            ))
            .await
    }
    ...
    ) -> Result<CompositionInfo, ApiError> {
    ...
    pub async fn execute(
        &self,
        id: &str,
        req: &ExecuteCompositionRequest,
    ) -> Result<CompositionExecutionResponse, ApiError> {
        self.client
            .post(&ApiEndpoints::COMPOSITION_EXECUTE.replace("{id}", id), req)
            .await
    }
}
